...
void SpriteRenderer::Draw(sf::RenderTarget& surface){
    ...
    if(show_outline){
        ...
        AssetManager::GetShader("Ligh")->setUniform("u_texture_pixel_step", 
            sf::Vector2f(1 / (float)sprite.getTexture()->getSize().x,
                        1 / (float)sprite.getTexture()->getSize().y));
        AssetManager::GetShader("Ligh")->setUniform("u_colour", sf::Color());

        surface.draw(sprite, AssetManager::GetShader("Ligh"));
    }
    else{
        surface.draw(sprite);
    }
